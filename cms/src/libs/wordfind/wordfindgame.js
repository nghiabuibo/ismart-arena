...
(function (document, $, wordfind) {
  ...
  */
  var WordFindGame = function (puzzleEl, puzzle, wordList) {
    ...
    var endTurn = function () {
      ...
      for (var i = 0, len = wordList.length; i < len; i++) {
        ...
        if (wordList[i] === curWord) {
          ...
          $(document).trigger('wordFound', curWord)
        }
    ...
    */
    ...
    this.solve = function(words=[], timeout=false) {
      words = words.length ? words : wordList
      ...
      for( var i = 0, len = solution.length; i < len; i++) {
        ...
        if (!wordEl.hasClass('wordFound')) {
          for (var j = 0, size = word.length; j < size; j++) {
            ...
            $('[x="' + nextPos.x + '"][y="' + nextPos.y + '"]').addClass(!timeout ? 'found' : 'solved');
          }

          wordEl.addClass('wordFound');

          const wordIndex = wordList.indexOf(word)
          if (wordIndex >= 0 && !timeout) {
            wordList.splice(wordIndex,1);
          }
        }
      }

      if (!timeout && wordList.length === 0) {
        $('.puzzleSquare').addClass('complete');
      }
    };
  };
